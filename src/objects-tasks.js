...
function shallowCopy(obj) {
  return { ...obj }; // Alternatively: Object.assign({}, obj)
}
...
function mergeObjects(objects) {
  return objects.reduce((acc, obj) => {
    Object.entries(obj).forEach((element) => {
      acc[element[0]] = (acc[element[0]] || 0) + obj[element[0]];
    });
    return acc;
  }, {});
}
...
function compareObjects(obj1, obj2) {
  let answer = Object.keys(obj1).length === Object.keys(obj2).length;
  if (answer) {
...
const cssSelectorBuilder = {
  str: [],
  element(value) {
    if (this.str.some((el) => el[0] === 'element')) {
      throw new Error(DUPLICATE_ERROR);
    }
    if (this.str.length > 0) {
      throw new Error(ORDER_ERROR);
    }
    return { ...this, str: this.str.concat([['element', value]]) };
  },
  id(value) {
    if (
      this.str.length > 0 &&
      !order.slice(0, 3).includes(this.str[this.str.length - 1][0])
    ) {
      throw new Error(ORDER_ERROR);
    }
    if (this.str.some((el) => el[0] === 'id')) throw new Error(DUPLICATE_ERROR);
    return { ...this, str: this.str.concat([['id', `#${value}`]]) };
  },

  class(value) {
    if (
      this.str.length > 0 &&
      !order.slice(0, 4).includes(this.str[this.str.length - 1][0])
    ) {
      throw new Error(ORDER_ERROR);
    }
    return { ...this, str: this.str.concat([['class', `.${value}`]]) };
  },

  attr(value) {
    if (
      this.str.length > 0 &&
      !order.slice(0, 5).includes(this.str[this.str.length - 1][0])
    ) {
      throw new Error(ORDER_ERROR);
    }
    return { ...this, str: this.str.concat([['attr', `[${value}]`]]) };
  },

  pseudoClass(value) {
    if (
      this.str.length > 0 &&
      !order.slice(0, 6).includes(this.str[this.str.length - 1][0])
    ) {
      throw new Error(ORDER_ERROR);
    }
    return { ...this, str: this.str.concat([['pseudoClass', `:${value}`]]) };
  },

  pseudoElement(value) {
    if (
      this.str.length > 0 &&
      !order.slice(0, 7).includes(this.str[this.str.length - 1][0])
    ) {
      throw new Error(ORDER_ERROR);
    }
    if (this.str.some((el) => el[0] === 'pseudoElement')) {
      throw new Error(DUPLICATE_ERROR);
    }
    return { ...this, str: this.str.concat([['pseudoElement', `::${value}`]]) };
  },
